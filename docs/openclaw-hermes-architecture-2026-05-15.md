...
- 2026-05-15 12:36 PDT update: repo fast-forwarded to upstream `NousResearch/hermes-agent` main `931caf2b2` (`fix(env-flags): widen truthy-only session env checks to sibling sites`), `uv pip install -e ".[cron,cli,mcp,honcho,messaging]"` completed, `hermes version` reports `Up to date`.
- `hermes doctor`:
  - core packages OK;
